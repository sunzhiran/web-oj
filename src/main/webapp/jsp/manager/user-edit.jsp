...
<div class="template-page-wrapper">
    ...
    <div class="templatemo-content-wrapper">
        <div class="templatemo-content">
            <h1>修改学生信息</h1>
            <%
                User userEdited = (User) request.getAttribute("user");
                List<StuClass> allClasses= (List<StuClass>) request.getAttribute("allClasses");
                boolean isWomen = "女".equals(userEdited.getUserSex());
                boolean isStudent = (1==userEdited.getUserType());
            %>
            <div class="row">
                <div class="col-md-12">
                    <form role="form" id="templatemo-preferences-form" onsubmit="return checkStudentEdit();" method="post" action="<%=basePath %>updateUser">
                        <div class="row">
                            <div class="col-md-6 margin-bottom-15">
                                <label for="userId" class="control-label">学号</label>
                                <input type="text" class="form-control" id="userId" name="userId" readonly="readonly" value="<%=userEdited.getUserId()%>">
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-6 margin-bottom-15">
                                <label for="username" class="control-label">姓名</label>
                                <input type="text" class="form-control" id="username" name="username" value="<%=userEdited.getUserName()%>">
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-6 margin-bottom-15">
                                <label for="password" class="control-label">密码</label>
                                <input type="text" class="form-control" id="password" name="password" value="<%=userEdited.getUserPassword()%>">
                            </div>
                        </div>
                        <div class="row">
                        ...
                        <div class="row">
                            <div class="col-md-6 margin-bottom-15">
                                <label for="singleSelect">专业</label>
                                <select class="form-control margin-bottom-15" id="singleSelect" name="chooseClass">
                                    <%
                                        if (allClasses != null) {
                                            for (int i=0; i<allClasses.size(); i++) {
                                                boolean isChoose = allClasses.get(i).getClassId().equals(userEdited.getUserClass());
                                    %>
                                    <option value="<%=allClasses.get(i).getClassId()%>" <%=!isChoose?"":"selected=\"selected\""%>><%=allClasses.get(i).getClassName()%></option>
                                    <%
                                            }
                                        }
                                    %>
                        ...
                        <div class="row">
                            <div class="col-md-6 margin-bottom-15">
                                <label for="userNote" class="control-label">备注</label>
                                <%
                                    String note = userEdited.getUserNote();
                                    if(null == note){
                                        note = "";
                                    }
                                %>
                                <input type="text" class="form-control" id="userNote" name="userNote" value="<%=note%>">
                            </div>
                        </div>
