...
<%
    ...
    List<RBACAccess> accesses = (List<RBACAccess>) session.getAttribute("accesss");
    if (accesses == null || accesses.size() <= 0) {
        permission = false;
    } else {
        for (RBACAccess access : accesses) {
            if ("/jsp/role.jsp".equalsIgnoreCase(access.getAccessURL())) {
                permission = true;
                break;
            }
        }
    ...
    if ( ! permission) {
        request.setAttribute("pageName", "/jsp/role.jsp");
        request.getRequestDispatcher("/jsp/no-permission.jsp").forward(request, response);
    }
                ...
                <div class="col-md-12">
                    ...
                    <div class="table-responsive">
                        <ul class="nav nav-pills">
                            <li><a href="/web-oj/jsp/role-add.jsp">添加角色 </a></li>
                        </ul>
                        <%
                            List<RBACRole> allRoles = (List<RBACRole>) request.getAttribute("allRoles");
                        ...
                        <%
                            ...
                            int numOfPage=allRoles.size()/numOfUserOnePage;
                            numOfPage = allRoles.size()%numOfUserOnePage==0 ? numOfPage:numOfPage+1;
                            ...
                            end = end>allRoles.size()? allRoles.size():end;
                        %>
                        <table class="table table-striped table-hover table-bordered">
                            ...
                            %>
                            <tr>
                                <td ><%=allRoles.get(i).getRoleId() %></td>
                                <td ><%=allRoles.get(i).getRoleName() %></td>
                                <td><a href="javascript:editRoleAccess(<%= allRoles.get(i).getRoleId() %>)" class="btn btn-default">编辑</a></td>
                            </tr>
                            <%
                                }
                            %>
